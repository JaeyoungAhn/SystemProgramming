/* spwd2.c: a simplified version of pwd
 *
 *		starts in current directory and recursively
 *		climbs up to /home of filesystem, prints top part
...
/*
 *		prints path leaing down to an object with this inode
 *		kindof recursive
 */
{
	ino_t my_inode;
	char its_name[BUFSIZ];
	if ( get_inode("..") != get_inode(".")) { /* when run from dir not a subdir of home, stops at root */
		if ( get_inode("..") != get_inode( "/home" ) ) { /* when run from subdir of home, stops right bf home */
		chdir( ".." );		/* up one dir */
